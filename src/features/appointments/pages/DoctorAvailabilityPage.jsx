...
import { setDoctorAvailability, fetchDoctorAvailability, deleteAvailabilitySlot, updateAvailabilitySlot } from "@/api/api";
import { Spinner, Modal, ModalActions } from "@/components/ui"; 
...
const DoctorAvailabilityPage = () => {
        ...
        existingAvailability.forEach(slot => {
          ...
          if (day && DAYS.includes(day)) {
            transformedAvailability[day].slots.push({
              ...
              isAvailable: slot.isAvailable !== false, // Read isAvailable from DB, default to true
              isSaved: true // MARK AS SAVED
            });
          }
        });

        // Determine day-level isAvailable based on whether any slot is active
        DAYS.forEach(day => {
          transformedAvailability[day].isAvailable = transformedAvailability[day].slots.some(
  ...
  const addSlot = () => {
    // New slots do NOT have the id/isSaved flag, default isAvailable to true
    setModalSlots((prev) => [...prev, { startTime: "", endTime: "", isAvailable: true }]);
  ...
  const editSlot = (idx, field, value) => {
    setModalSlots((prev) => {
      ...
      // Recalculate modalAvailable based on whether any slot is active
      const hasActiveSlot = updatedSlots.some(s => s.isAvailable !== false);
      setModalAvailable(hasActiveSlot);

      return updatedSlots;
    });
  };

  // --- UPDATED: Modal Slot Deletion Logic (Handles custom Modal confirmation) ---
  const deleteSlot = (idx) => {
    ...
    if (isSavedSlot) {
      setSlotIndexToDelete(idx);
      setDeleteConfirmOpen(true);
    } else {
      // For local unsaved slots, delete directly
      setModalSlots((prev) => {
        const filtered = prev.filter((_, i) => i !== idx);
        const hasActiveSlot = filtered.some(s => s.isAvailable !== false);
        setModalAvailable(hasActiveSlot);
        return filtered;
      });
    }
  ...
  const confirmDeleteSavedSlot = async () => {
    if (slotIndexToDelete === null) return;
    
    const idx = slotIndexToDelete;
    const slotToDelete = modalSlots[idx];
    const slotId = slotToDelete.id;
  ...
  const copyToAllDays = (day) => {
    setAvailability((prev) => {
      ...
      return DAYS.reduce((acc, d) => {
        ...
        acc[d] = { isAvailable, slots: newSlots };
        return acc;
      }, {});
    });
    setCopyDay(day);
  ...
  // Helper: format time with seconds if needed
  ...
  const handleSaveAll = async () => {
    ...
    DAYS.forEach((day) => {
      availability[day].slots.forEach((slot) => {
        if (slot.startTime && slot.endTime) {
          ...
          if (!slot.isSaved) {
            newSlotsPayload.push(formatted);
          } else if (slot.isEdited) {
            editedSlotsPayload.push({
              id: slot.id,
              data: formatted
            });
          }
    ...
    if (newSlotsPayload.length === 0 && editedSlotsPayload.length === 0) {
    ...
    try {
      ...
      if (newSlotsPayload.length > 0) {
        await setDoctorAvailability(user.userId, newSlotsPayload);
      }

      // 2. Update edited slots (multiple PUT requests if needed)
      if (editedSlotsPayload.length > 0) {
        await Promise.all(
          editedSlotsPayload.map(slot => 
            updateAvailabilitySlot(user.userId, slot.id, slot.data)
          )
        );
      }
      ...
      <div className="max-w-6xl mx-auto mt-8 lg:mt-12 px-4 lg:px-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 lg:p-8">
          <div className="flex justify-center items-center h-32">
            <Spinner size="md" text="Loading your availability..." />
          </div>
        </div>
      </div>
    );
    ...
    <div className="max-w-6xl mx-auto mt-8 lg:mt-12 px-4 lg:px-6 pb-8">
            ...
            >
              ...
              <div className="flex items-center justify-between lg:justify-start mb-3 lg:mb-0">
                ...
                <input
                  ...
                  onChange={() => {
                    const newIsAvailable = !availability[day].isAvailable;
                    setAvailability((prev) => {
                      const dayData = prev[day];
                      let updatedSlots = [];
                      if (newIsAvailable) {
                        // Turning ON: if no slots, add an empty active slot. Else activate all slots.
                        if (dayData.slots.length === 0) {
                          updatedSlots = [{ startTime: "", endTime: "", isAvailable: true }];
                        } else {
                          updatedSlots = dayData.slots.map(slot => ({
                            ...slot,
                            isAvailable: true,
                            isEdited: slot.isSaved ? true : slot.isEdited
                          }));
                        }
                      } else {
                        // Turning OFF: deactivate all slots
                        updatedSlots = dayData.slots.map(slot => ({
                          ...slot,
                          isAvailable: false,
                          isEdited: slot.isSaved ? true : slot.isEdited
                  ...
                  <div className="flex flex-wrap gap-2">
                    {availability[day].slots.map((slot, idx) => {
                      ...
                      return (
                        <span 
                          key={idx} 
                          className={`inline-block px-3 py-1 rounded-full text-sm font-medium transition-all duration-200 ${
                            isActive 
                        ...
                        >
                          {slot.startTime && slot.endTime 
                            ? `${formatTimeToAMPM(slot.startTime)} - ${formatTimeToAMPM(slot.endTime)}${!isActive ? " (Inactive)" : ""}` 
                            : "(edit slots)"
                          }
                        </span>
                      );
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-auto relative animate-fadeIn max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-4 lg:p-6">
              <ToggleSwitch
                ...
                onChange={() => {
                  const newVal = !modalAvailable;
                  setModalAvailable(newVal);
                  setModalSlots((prev) =>
                    prev.map((slot) => ({
                      ...slot,
                      isAvailable: newVal,
                      ...
                      <div className="flex-1 flex flex-col gap-3">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-gray-200/60">
                          <ToggleSwitch
                            ...
                            onChange={() => editSlot(idx, "isAvailable", !(slot.isAvailable !== false))}
                            label="Active Slot"
                          />
                        </div>
                      </div>
        ...
        footer={
          <ModalActions>
            <button
              ...
              className="px-4 py-2 border border-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-50 active:scale-95 transition-all duration-200"
            >
              Cancel
            </button>
            <button
              onClick={confirmDeleteSavedSlot}
              className="px-4 py-2 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 active:scale-95 transition-all duration-200 shadow-md shadow-red-200"
            >
              Remove Slot
            </button>
          </ModalActions>
      ...
      >
        <p className="text-gray-600 leading-relaxed">
          Are you sure you want to remove this availability slot? Patients will no longer be able to book appointments during this time.
        </p>
      </Modal>
