...
const DoctorDashboard = () => {
  ...
  const fetchTodayCount = useCallback(async () => {
    ...
    try {
      const response = await fetchDoctorAppointments(user.userId, {
        page: 0,
        size: 1,
        sort: "appointmentDate,startTime,desc",
        status: "ALL",
        dateFilter: "today"
      });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetchDoctorProfile()
    ]).then(([apptsData, todayCount, avail, prof]) => {
      setAppointments(apptsData.appointments);
      setTotalPages(apptsData.totalPages);
      setTotalElements(apptsData.totalElements);
      setAppointmentsTodayCount(todayCount);
      setAvailability(avail);
      setProfile(prof);
      setLoading(false);
    }).catch(() => setLoading(false));
  }, [user, fetchAppointments, fetchTodayCount, currentPage, currentSort, currentStatus, currentDateFilter]);
